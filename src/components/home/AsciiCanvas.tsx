...
export default function AsciiCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Draw salon image inside moon circle (behind ASCII)
      if (imgRef.current) {
        ctx.save();
        ctx.beginPath();
        ctx.arc(moonX, moonY, moonRadius * 0.92, 0, Math.PI * 2);
        ctx.clip();

        // Image distortion based on mouse
        const mouseDistance = Math.hypot(mouse.x - moonX, mouse.y - moonY);
        const mouseField = Math.exp(-mouseDistance * 0.004);
        const imgScale = 1 + mouseField * 0.08;
        const imgShiftX = (mouse.x - moonX) * mouseField * 0.03;
        const imgShiftY = (mouse.y - moonY) * mouseField * 0.03;

        const imgSize = moonRadius * 2.1 * imgScale;
        ctx.drawImage(
          imgRef.current,
          moonX - imgSize / 2 + imgShiftX,
          moonY - imgSize / 2 + imgShiftY,
          imgSize,
          imgSize
        );

        // Darken overlay for ASCII readability
        ctx.fillStyle = `rgba(0, 0, 0, ${0.35 + mouseField * 0.15})`;
